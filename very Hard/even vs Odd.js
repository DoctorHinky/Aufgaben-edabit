...
function availableSpots(array, num) {
    let likes = 0;
    ...
    for (let i = 0; i < array.length - 1; i++) {
        ...
        if(isLiked(num, left, right)){
            likes++;
        }
    }
    return likes;
}
function isLiked(num, left, right) {
    const eveNum = num % 2 === 0;   // Ist num gerade?
    const eveLeft = left % 2 === 0;  // Ist left gerade?
    const eveRight = right % 2 === 0; // Ist right gerade?

    if (eveNum) {

        return (eveLeft && eveRight);
    } else {
        
        return (left % 2 !== 0 && right % 2 !== 0); 
    }
}
console.log(availableSpots([0, 4, 6, 8], 9)); // 3
console.log(availableSpots([0, 4, 6, 8], 12)); // 0
